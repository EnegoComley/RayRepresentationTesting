...
class InterpolatableGridReconstruction(L.LightningModule):
    ...
    def calculate_loss(self, batch, stage):
        ...
        rgbs, raw_rgb = self.RayManager.get_colour(rgb_xyz_sampled, colour_reconstruction, rgb_weight, False, give_raw_rgb=True)
        ...
        center_loss = self.loss_func(center_opacity, torch.ones_like(center_opacity))
        ...
        if self.loss_method == "WO":
            ...
            final_loss = density_loss * 169.6882 + opacity_loss * 169.6882 + mask_rgb_loss
        elif self.loss_method == "DO+CRGB":
            final_loss = density_loss + opacity_loss + mask_colour_loss + mask_rgb_loss
        elif self.loss_method == "WDO+CRGB":
        ...
        elif self.loss_method == "DO+CRGB+SplitRay":
            final_loss = density_loss + opacity_loss + mask_colour_loss + mask_rgb_loss + raw_ray_colour_loss + edge_loss + center_loss
        elif self.loss_method == "DO+CRGB+SplitRay+Dice":
            final_loss = density_loss + opacity_loss + mask_colour_loss + mask_rgb_loss + raw_ray_colour_loss + edge_loss + center_loss + dice_loss
        else:
...
if __name__ == "__main__":
    ...
    batch_size_dict = {"BS" : 1, "acc" : 1} if args.no_logger else {1 : {"BS" : 3, "acc" : 10}, 2 : {"BS" : 2, "acc" : 15}, 3 : {"BS" : 1, "acc" : 30}, 4 : {"BS" : 1, "acc" : 30}, 5 : {"BS" : 1, "acc" : 30}, 12 : {"BS" : 1, "acc" : 30}}[args.scale]
    if args.overfit:
        batch_size_dict = {"BS" : 1, "acc" : 20}
